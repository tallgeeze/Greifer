...
def getData(fname):
    ...
    try:
        while line != '':
            split = line.split(',')
            data['g_type'].append(int(split[1]))
            data['time1'].append(float(split[2]))
            data['time2'].append(float(split[3]))
            data['vorschub'].append(float(split[4]))
            data['erfolg'].append(int(split[5]))
            line = f.readline()
    except:
        pass
    return data
...
###############################    
examples = buildExamples('g1_data.txt')   #here insert file name 
###############################
...
def lr(trainingData, testData, prob = 0.50):    #to change thread of judgement
